...
public class AudioWaveRecorder {
	private final static int[] sampleRates = { 8000 };

	public static AudioWaveRecorder getInstanse(Boolean recordingCompressed) {
		AudioWaveRecorder result = null;

		if (recordingCompressed) {
			result = new AudioWaveRecorder(false, AudioSource.MIC,
					sampleRates[3], AudioFormat.CHANNEL_CONFIGURATION_MONO,
					AudioFormat.ENCODING_PCM_16BIT);
		} else {
			int i = 0;
			do {
				result = new AudioWaveRecorder(true, AudioSource.MIC,
						sampleRates[i], AudioFormat.CHANNEL_CONFIGURATION_MONO,
						AudioFormat.ENCODING_PCM_16BIT);

			} while ((++i < sampleRates.length)
					& !(result.getState() == AudioWaveRecorder.State.INITIALIZING));
		}
		return result;
	}
	 ...
	 * 
	 */
	public AudioWaveRecorder(boolean uncompressed, int audioSource,
